...
本地按可配置周期检查已接入仓库的新增内容，不用 Actions 定时扫描，也不为每次空检查调用模型。`runtime.pollSeconds` 的 V1 默认值为 300 秒（5 分钟），部署者可以配置其他正整数；读取增量而非反复重读全部 Issue，初次启动不把历史命令全部重放。一次 polling cycle 最多新增领取一个 Issue；完整规则见 [Harness 并发与轮询调度](04-harness-scheduling.md)。
...
同一份 GitHub 内容事件不重复执行，同一任务不同时启动两个写入者。任务正在执行时不能另开进程或会话抢写同一分支；不同任务可以在机器级和仓库级并发上限允许时并行。没有空槽时不得领取或消费新的有效触发；Runner 重启后无法确认旧 Harness 是否结束时，该调用保守继续占槽，直到取得确定结果。完整规则见 [Harness 并发与轮询调度](04-harness-scheduling.md)。多台电脑的 Runner 名称由部署者保持唯一，不建设分布式选主。
