...
过程通过终端／本机日志观察。官方 `--json` stdout 是 JSONL 事件流，接单工具只解析 session、turn_end、final/error 等必要控制事件；模型正文、thinking、tool result 与原始 stdout/stderr 只留本机，不作为公开状态转述。使用当前版本实际提供的输出与退出状态，不承诺逐 token 展示，不新建 Web 界面。明确区分已接单、实际执行中、调用／执行错误、等待维护者和 Dev 报告完成；进程成功退出或出现最终回答，不单独代表测试、Review 或业务验收通过。工具报告自己可核验的进程状态，业务完成与待决事项由 Dev 按目标项目规则报告，不新增模型裁判。
