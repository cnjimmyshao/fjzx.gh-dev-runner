...
维护者选择简化部署：模型 Key 由 Harness 当前版本支持的本机凭据／配置机制保存，小接单工具直接启动 Harness headless CLI，不再依赖已有 `dsh web`。原 Web 实验说明另一条路径的调查结果，并非证明 CLI 方案已通过；本次变化来自维护者取舍，不是实验推翻。
...
Harness 负责保存和保护自己的模型凭据；小接单工具只保存 Runner 自身的部署配置、任务绑定与处理进度，并读取真实进程结果。Runner 不建立第二份模型凭据存储，也不把完整 Runner 环境无差别透传给 Harness；不重写 Agent 循环、审查裁判或自动恢复系统。保留授权、去重、单任务单写入者和不自动合并的边界。
